...
export const updatePlantSensors = async (plantUniqueId, sensorValues) => {
    try {
        const response = await databases.updateDocument(
            config.databaseId, config.plantsCollectionId, plantUniqueId, {
...
export const subscribeToPlants = (userId, callback) => {
    try {
        const unsubscribe = client.subscribe(`databases.${config.databaseId}.collections.${config.plantsCollectionId}.documents`, (response) => {
